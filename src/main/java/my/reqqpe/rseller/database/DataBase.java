...
public class DataBase {
    ...
    public DataBase(RSeller plugin) {
        this.plugin = plugin;
        try {
    ...
    public PlayerData getPlayerData(UUID uuid) {
        return playersData.computeIfAbsent(uuid, PlayerData::new);
    }
    ...
    public void loadPlayerData(UUID uuid) {
        String sql = "SELECT points, level, autosell FROM players WHERE uuid=?";
    ...
    public void savePlayerData(UUID uuid) {
        String sql;
        switch (dbType) {
            case MYSQL -> sql = "INSERT INTO players (uuid, points, level, autosell) VALUES (?,?,?,?) " +
                    "ON DUPLICATE KEY UPDATE points=VALUES(points), level=VALUES(level), autosell=VALUES(autosell)";
            case POSTGRESQL -> sql = "INSERT INTO players (uuid, points, level, autosell) VALUES (?,?,?,?) " +
                    "ON CONFLICT (uuid) DO UPDATE SET points=EXCLUDED.points, level=EXCLUDED.level, autosell=EXCLUDED.autosell";
            default -> sql = "INSERT OR REPLACE INTO players (uuid, points, level, autosell) VALUES (?,?,?,?)";
        }
        PlayerData data = getPlayerData(uuid);
        try (Connection c = dataSource.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
    ...
    public void saveAll() {
        for (PlayerData data : playersData.values()) savePlayerData(data.getUuid());
